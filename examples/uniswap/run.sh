...
# "Swap TEST for WETH", from the repository root.
...
### Defaults from README (can be overridden by environment) ###

export RPC_URL="${RPC_URL:-http://localhost:1729}"
export RICH_SK_L2="${RICH_SK_L2:-0xe4f7dc8b199fdaac6693c9c412ea68aed9e1584d193e1c3478d30a6f01f26057}"
export DETERMINISTIC_DEPLOYER="${DETERMINISTIC_DEPLOYER:-0x4e59b44847b379578588920ca78fbf26c0b4956c}"
export WETH_ADDRESS="${WETH_ADDRESS:-0x000000000000000000000000000000000000FfFD}"

# Known addresses from README (derived from the above private keys)
RICH_ADDRESS="${RICH_ADDRESS:-0x0000bd19F707CA481886244bDd20Bd6B8a81bd3e}"

# TEST token fixed address per deterministic deployer + salt 0
export TEST_TOKEN_ADDRESS="${TEST_TOKEN_ADDRESS:-0xB66dd10F098f62141A536e92f6e8f7f9633893E2}"

# Empty account used for the final swap
EMPTY_ACCOUNT_ADDRESS="${EMPTY_ACCOUNT_ADDRESS:-0x41F31fBf85a69c9F3a1635bBF8F602F6e78F3aDF}"
EMPTY_ACCOUNT_SK="${EMPTY_ACCOUNT_SK:-0xdd5fcfb45b5702ba0b5c326d0fa29b28dfe4854e3fbd4e104bfae90cefe7732e}"

# Deploy-v3 deployer as per README
DEPLOY_V3_PK="${DEPLOY_V3_PK:-0x1bc8b78019f35d4447a774e837d414a3db9e1dea5cfc4e9dc2fc3904969ab51f}"
...
POOL_A=$(rex call "$FACTORY_ADDRESS" "getPool(address,address,uint24)" "$WETH_ADDRESS" "$TEST_TOKEN_ADDRESS" 3000 | extract_abi_address)
POOL_B=$(rex call "$FACTORY_ADDRESS" "getPool(address,address,uint24)" "$TEST_TOKEN_ADDRESS" "$WETH_ADDRESS" 3000 | extract_abi_address)

if [[ -n "$POOL_A" && "$POOL_A" != "0x0000000000000000000000000000000000000000" ]]; then
  LIQUIDITY_POOL_ADDRESS="$POOL_A"
elif [[ -n "$POOL_B" && "$POOL_B" != "0x0000000000000000000000000000000000000000" ]]; then
  LIQUIDITY_POOL_ADDRESS="$POOL_B"
else
  die "Could not determine liquidity pool address (A='$POOL_A', B='$POOL_B')"
fi
